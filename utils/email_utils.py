...
from models import ROLE_SYSTEM_ADMIN, ROLE_ADMINISTRATOR
from utils.action_tokens import make_ticket_action_token
...
# ── Role-aware ticket action links ──────────────────────────────────────────
# Each button is a signed one-click link (see utils/action_tokens.py) scoped
# to one staff member and one ticket. Clicking it signs that staff member in
# and deep-links them straight to the ticket in the dashboard, already
# focused on the action — it never performs the action by itself, so which
# buttons a given recipient actually sees is what makes this "role-aware":
# the same can_manage_ticket()/can_assign() checks the dashboard and API
# already enforce decide the button set here too.
def _email_action_url(staff_id, ticket_id, focus=None):
    base = current_app.config.get('BASE_URL', '')
    if not base or not staff_id or not ticket_id:
        return ''
    token = make_ticket_action_token(staff_id, ticket_id, focus)
    return f"{base.rstrip('/')}/admin/go/{token}"


def _action_buttons_html(buttons):
    """buttons: list of (url, label_ar, label_en, color). Renders a row of
    side-by-side buttons; skips any entry with no url."""
    buttons = [b for b in buttons if b and b[0]]
    if not buttons:
        return ''
    cells = ''.join(
        f'<td style="padding:4px 6px">'
        f'<a href="{url}" style="display:inline-block;background:{color};color:#fff;text-decoration:none;'
        f'padding:9px 16px;border-radius:6px;font-weight:700;font-size:12px;white-space:nowrap" dir="ltr">{label_ar} / {label_en}</a>'
        f'</td>'
        for url, label_ar, label_en, color in buttons
    )
    return f"""
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:14px auto"><tr>{cells}</tr></table>"""


def _ticket_action_buttons_for_staff(staff, ticket_id):
    """Builds the role-appropriate action-button set for one staff member
    on one ticket (used for the brand-new, still-unassigned ticket
    broadcast): everyone in the recipient list can open it; only
    system_admin/administrator get Assign/Comment buttons — for an
    unassigned ticket that's exactly what can_manage_ticket()/can_assign()
    resolve to, since technical_support only manages tickets already
    assigned to them. The two comment buttons don't post anything by
    themselves (a comment needs real text) — they deep-link into the same
    comment box the dashboard already has, with the internal/public
    checkbox pre-set, so replying (of either kind) needs one click to get
    there instead of hunting the ticket down first."""
    buttons = [(
        _email_action_url(staff.id, ticket_id), 'فتح التذكرة', 'Open Ticket', '#2E4E7A',
    )]
    if staff.role in (ROLE_SYSTEM_ADMIN, ROLE_ADMINISTRATOR):
        buttons.append((
            _email_action_url(staff.id, ticket_id, 'assign'), 'تعيين', 'Assign', '#247680',
        ))
        buttons.append((
            _email_action_url(staff.id, ticket_id, 'comment'), 'تعليق عام', 'Public Comment', '#247680',
        ))
        buttons.append((
            _email_action_url(staff.id, ticket_id, 'comment-internal'), 'ملاحظة داخلية', 'Internal Note', '#6b7c7e',
        ))
    return _action_buttons_html(buttons)
...
def send_ticket_assigned(t):
    """Sent to the staff member a ticket was (re)assigned to. Buttons: the
    assignee can always open + close it (they're the assignee, so
    can_manage_ticket() is always true for them); an Assign button is
    added only if their role can also reassign it (can_assign())."""
    assignee_id = t.get('assigneeId')
    buttons_list = [(
        _email_action_url(assignee_id, t['id']), 'فتح التذكرة', 'Open Ticket', '#2E4E7A',
    ), (
        _email_action_url(assignee_id, t['id'], 'close'), 'إغلاق التذكرة', 'Close Ticket', '#27ae60',
    )]
    if t.get('assigneeRole') in (ROLE_SYSTEM_ADMIN, ROLE_ADMINISTRATOR):
        buttons_list.append((
            _email_action_url(assignee_id, t['id'], 'assign'), 'إعادة تعيين', 'Reassign', '#247680',
        ))
    buttons = _action_buttons_html(buttons_list)
    content_ar = f"""
    <h2 style="color:#247680;margin-top:0">📌 تم تعيين تذكرة لك</h2>
    <table style="width:100%;border-collapse:collapse;margin:16px 0">{_rows_ar(t)}</table>
    <p style="color:#555">يمكنك فتح التذكرة أو إغلاقها مباشرة من الأزرار أدناه.</p>
    {buttons}
    """
    content_en = f"""
...
def send_staff_new_ticket_notification(t, staff_list):
    """Broadcast to staff (e.g. all System Admins) when a new ticket comes
    in, so someone assigns it promptly. `staff_list` is a list of
    StaffUser objects (not bare emails) — each recipient gets their own
    action buttons, scoped to their own account and role (see
    _ticket_action_buttons_for_staff)."""
    subject = f"[إشعار/Notice] تذكرة جديدة #{t['serial']}"
    for staff in staff_list:
        if not staff or not staff.email or '@' not in staff.email:
            continue
        buttons = _ticket_action_buttons_for_staff(staff, t['id'])
        content_ar = f"""
        ...
        try:
            _send(staff.email, staff.full_name or '', subject, html)
        except Exception as e:
            print(f"[email] staff new-ticket notification failed for {staff.email}: {e}", flush=True)
