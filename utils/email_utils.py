  ...
  <tr><td align="center" style="padding:16px 10px">
    <table role="presentation" width="480" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr><td style="background:#f5f5f5;padding:8px 16px;text-align:center;font-size:10px;color:#888">
        <div dir="rtl">هذا البريد مُرسَل تلقائياً من نظام تذاكر الدعم الفني — يُرجى عدم الرد عليه</div>
...
def send_ticket_merged(t, kept_serial):
    content_ar = f"""
...
def send_new_comment_notification(t, comment, to_email, to_name):
    ...
    page open."""
    author_ar = 'الدعم الفني' if comment['authorRole'] == 'staff' else t['teacherName']
    author_en = 'IT Support' if comment['authorRole'] == 'staff' else t['teacherName']
    content_ar = f"""
