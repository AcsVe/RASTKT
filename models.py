...
class Teacher(db.Model):
    ...
    here (see utils.helpers.match_teacher)."""
...
class CategoryItem(db.Model):
    ...
    Projector... each with its own identifier/code."""
    __tablename__ = 'category_items'
    id          = db.Column(db.Integer, primary_key=True)
    category_id = db.Column(db.Integer, db.ForeignKey('categories.id'), nullable=False)
    code        = db.Column(db.String(50))       # admin-assigned identifier
    name_ar     = db.Column(db.String(150), nullable=False)
    name_en     = db.Column(db.String(150))
    active      = db.Column(db.Boolean, default=True)
    sort_order  = db.Column(db.Integer, default=0)
