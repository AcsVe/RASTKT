...
from models import (db, Ticket, TicketComment, Teacher, Category, CategoryItem,
                     gen_serial_number, gen_tracking_code, get_setting)
...
def new_ticket():
    lang = _lang()
    allow_external = get_setting('allow_external_submitters', '0') == '1'

    if request.method == 'GET':
        categories = Category.query.filter_by(active=True).order_by(Category.sort_order).all()
        return render_template('ticket_form.html', lang=lang, allow_external=allow_external,
    ...
    manual_email = sanitize_text(request.form.get('manual_email'))
    ...
    if not teacher:
        if allow_external and teacher_name and is_valid_email(manual_email):
            # Reuse an existing guest row for the same name+email if one
            # exists, otherwise create a new (roster-hidden) one.
            external_teacher = (Teacher.query
                                 .filter_by(external=True)
                                 .filter(db_ilike(Teacher.name, teacher_name))
                                 .filter(db_ilike(Teacher.email, manual_email))
                                 .first())
            if not external_teacher:
                external_teacher = Teacher(name=teacher_name, email=manual_email, active=True, external=True)
                db.session.add(external_teacher)
                db.session.flush()
        elif allow_external and teacher_name and not is_valid_email(manual_email):
            errors['manual_email'] = ('الرجاء إدخال بريد إلكتروني صحيح' if lang == 'ar'
    ...
    if not subject:
        errors['subject'] = 'الرجاء كتابة موضوع مختصر للطلب' if lang == 'ar' else 'Please provide a short subject'

    if errors:
        db.session.rollback()
        categories = Category.query.filter_by(active=True).order_by(Category.sort_order).all()
        return render_template('ticket_form.html', lang=lang, errors=errors, allow_external=allow_external,
                                form=request.form,
                                categories=[c.to_dict(with_items=True) for c in categories]), 400

    active_teacher = teacher or external_teacher
    ...
    ticket = Ticket(
        ...
        teacher_name=active_teacher.name,
        teacher_email=active_teacher.email,
        teacher_phone=active_teacher.phone,
    ...
    try:
        ...
        notify_staff = [s for s in StaffUser.query.filter(
            StaffUser.role.in_([ROLE_SYSTEM_ADMIN, ROLE_ADMINISTRATOR]), StaffUser.active.isnot(False)
        ).all() if s.email]
        if notify_staff:
            send_staff_new_ticket_notification(_ticket_email_ctx(ticket), notify_staff)
    except Exception as e:
    ...
    if body:
        ...
        try:
            if ticket.assignee and ticket.assignee.email:
                ...
                for a in admins:
                    send_push_to_staff(app_obj, a.id,
...
def api_teacher_names():
    ...
    query = Teacher.query.filter(Teacher.active.isnot(False)).filter(Teacher.external.isnot(True))
    if q:
...
@public_bp.route('/api/ticker')
def api_ticker():
    """Public, unauthenticated — the scrolling ticker renders on every
    page (including for anonymous visitors), so its config must be
    readable without a login."""
    from utils.ticker_utils import build_ticker_payload
    return jsonify(build_ticker_payload())
