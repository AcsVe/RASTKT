...
from utils.email_utils import (send_ticket_assigned, send_ticket_closed, send_ticket_reopened,
                                send_ticket_merged, send_new_comment_notification)
...
def api_bulk_add_teachers():
    ...
    for row in items:
        ...
        if not name:
            errors.append(row)
            continue
        if Teacher.query.filter(db_ilike(Teacher.name, name)).first():
